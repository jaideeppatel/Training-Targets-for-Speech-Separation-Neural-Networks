...
for(attr in 1:(ncol(new_dataset)-1)){
  ...
  for (rows in 1:nrow(freqsets)){
    ...
    main1<-toString(as.vector(as.matrix(freqsets[rows,])))
    class(main1)
    main2<-noquote(main1)
    main3<-gsub(',','',main2)
    main<-as.character(as.vector(as.matrix(freqsets[rows,])))
    
    #Generationg rules from the frequent item set
    for(cols in iter:1){
      black_list<-c()
      rules1<-data.frame(combn(as.vector(as.matrix(freqsets[rows,])),cols))
      rules1<-t(rules1)
      ...
      for(ruler in 1:nrow(rules1)){
        ...
        antecedent1<-toString(antecedent)
        rules_set<-data.frame()
        rules_set<-rbind(rules_set,data.frame(Ante=antecedent1,Cons=consequent,Conf=NA))
        
        # check if the rule needs to be pruned or not
        # Fetch rule support
        conseq_chk<-setdiff(main,antecedent)
        
        if(any(conseq_chk%in%black_list)){
          rules_set<-rules_set[-nrow(rules_set),]
        } else {
          ...
          for (j in 1:nrow(new_dataset)){
            ...
            for (x in 1:cols){
              ...
              if(as.matrix(new_dataset[j,as.numeric(temp[x,1])])!=1){
                flag3=1
              }
            }
            if(flag3==0){
          ...
          # check for min confidence condition and add to black_list
          if(rule_confidence<min_conf){
            rules_set<-rules_set[-nrow(rules_set),]
            black_list<-c(black_list,conseq_chk)
            black_list<-unique(black_list)
          }
          else{
            myrules<-rbind(myrules,data.frame(rules_set[nrow(rules_set),]))
          }
        }
      }
    }
...
print(rules_set)
# Output only Frequent association rules
rules_set<-rules_set[which(rules_set$Sup>=sup),]
print('======== All Frequent Associoation Rules==========')
rules_set_xx <- rules_set[rev(order(rules_set$Conf)),]
print(rules_set_xx[1:10,])


print(rules_set)
rules_set$lift<-0
rules_set$Sup<-rules_set$Sup/nrow(new_dataset)
